...
function typeText(el, text, speed = 70, startDelay = 300) {
  ...
  setTimeout(() => {
    const timer = setInterval(() => {
      el.textContent += text[i] ?? "";   // ✅ undefined 방지
      i++;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const titleText = titleEl ? titleEl.dataset.text || titleEl.textContent : "";
  const subText   = subEl ? subEl.dataset.text || subEl.textContent : "";

  // 시작 전에 원문 저장(타이핑 중 textContent 바뀌어도 안전)
  if (titleEl) titleEl.dataset.text = titleText;
  if (subEl) subEl.dataset.text = subText;

  typeText(titleEl, titleText, 75, 250);
  typeText(subEl, subText, 45, 1400);
});
